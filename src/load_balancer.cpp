...
LoadBalancer::LoadBalancer(int lb_id,
                           ...
                           int fp_start_id)
    : lb_id_(lb_id), fp_start_id_(fp_start_id), num_fps_(fp_queues.size()),
      input_queue_(10000), fp_queues_(std::move(fp_queues)),
...
void LoadBalancer::start() {
  ...
  std::cout << "[LB" << lb_id_ << "] Started (serving FP" << fp_start_id_
...
LBManager::LBManager(int num_lbs, int fps_per_lb,
  ...
  for (int lb_id = 0; lb_id < num_lbs; lb_id++) {
    ...
    lbs_.push_back(
        std::make_unique<LoadBalancer>(lb_id, lb_fp_queues, fp_start));
  }
