...
FastPathProcessor::FastPathProcessor(int fp_id, RuleManager *rule_manager,
                                     PacketOutputCallback output_callback)
    : fp_id_(fp_id), input_queue_(10000), conn_tracker_(fp_id),
...
void FastPathProcessor::stop() {
  ...
  std::cout << "[FP" << fp_id_ << "] Stopped (processed " << packets_processed_
            << " packets)\n";
}
...
PacketAction FastPathProcessor::checkRules(const PacketJob &job,
  ...
  auto block_reason = rule_manager_->shouldBlock(src_ip, job.tuple.dst_port,
...
FPManager::FPManager(int num_fps, RuleManager *rule_manager,
                     PacketOutputCallback output_callback) {

  // Create FP processors (each has its own input queue)
  for (int i = 0; i < num_fps; i++) {
    auto fp =
        std::make_unique<FastPathProcessor>(i, rule_manager, output_callback);
...
std::string FPManager::generateClassificationReport() const {
  ...
  // Sort apps by count
  std::vector<std::pair<AppType, size_t>> sorted_apps(app_counts.begin(),
                                                      app_counts.end());
  ...
  for (const auto &pair : sorted_apps) {
    ...
    ss << "║ " << std::setw(15) << std::left << appTypeToString(pair.first)
       << std::setw(8) << std::right << pair.second << " " << std::setw(5)
       << std::fixed << std::setprecision(1) << pct << "% " << std::setw(20)
       << std::left << bar << "   ║\n";
  }
