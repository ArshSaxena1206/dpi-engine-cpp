...
void RuleManager::blockApp(AppType app) {
  ...
  std::cout << "[RuleManager] Blocked app: " << appTypeToString(app)
            << std::endl;
}
...
void RuleManager::unblockApp(AppType app) {
  ...
  std::cout << "[RuleManager] Unblocked app: " << appTypeToString(app)
            << std::endl;
}
...
void RuleManager::unblockDomain(const std::string &domain) {
  ...
  if (domain.find('*') != std::string::npos) {
    auto it =
        std::find(domain_patterns_.begin(), domain_patterns_.end(), domain);
...
bool RuleManager::domainMatchesPattern(const std::string &domain,
  ...
  if (pattern.size() >= 2 && pattern[0] == '*' && pattern[1] == '.') {
    ...
    if (domain.size() >= suffix.size() &&
        domain.compare(domain.size() - suffix.size(), suffix.size(), suffix) ==
            0) {
      return true;
    }
...
RuleManager::shouldBlock(uint32_t src_ip, uint16_t dst_port, AppType app,
