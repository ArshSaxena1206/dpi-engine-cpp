...
bool DPIEngine::initialize() {
  ...
  fp_manager_ =
      std::make_unique<FPManager>(total_fps, rule_manager_.get(), output_cb);

  // Create LB manager (creates LB threads, connects to FP queues)
  lb_manager_ = std::make_unique<LBManager>(config_.num_load_balancers,
                                            config_.fps_per_lb,
  ...
  for (int i = 0; i < total_fps; i++) {
    global_conn_table_->registerTracker(
        i, &fp_manager_->getFP(i).getConnectionTracker());
  }
  ...
  if (job.data.size() > 14) {
    ...
    if (parsed.has_tcp && job.data.size() > job.transport_offset) {
      uint8_t tcp_data_offset =
          (job.data[job.transport_offset + 12] >> 4) & 0x0F;
...
void DPIEngine::writeOutputPacket(const PacketJob &job) {
  ...
  output_file_.write(reinterpret_cast<const char *>(&pkt_header),
                     sizeof(pkt_header));
  output_file_.write(reinterpret_cast<const char *>(job.data.data()),
                     job.data.size());
}
...
/**
 * @brief Generates a comprehensive summary report of the DPI engine's
 * execution.
 *
 * 1. What it does:
 *    Compiles packet statistics, filtering metrics (dropped/forwarded), thread
 * performance, and application breakdown into a formatted ASCII table string.
 *
 * 2. Which modules depend on it:
 *    - Community 1 (DPI Engine Core): Calls this at the end of processFile().
 *    - Community 2 (Server/Backend): The stdout output of this function is
 * parsed by the Node.js backend.
 *
 * 3. What breaks if it changes:
 *    - The Node.js regex parsers in `packetJob.js` rely on EXACT string formats
 * like "Total Packets: " and "Dropped: ". Changing the output format will break
 * the frontend dashboard metrics.
 *
 * 4. Known edge cases:
 *    - Zero total packets will cause divide-by-zero if drop_rate calculation
 * isn't guarded.
...
std::string DPIEngine::generateReport() const {
  ...
  ss << "║   Forwarded:          " << std::setw(12)
     << stats_.forwarded_packets.load() << "                        ║\n";
  ss << "║   Dropped/Blocked:    " << std::setw(12)
     << stats_.dropped_packets.load() << "                        ║\n";

  if (stats_.total_packets > 0) {
    double drop_rate =
        100.0 * stats_.dropped_packets.load() / stats_.total_packets.load();
    ss << "║   Drop Rate:          " << std::setw(11) << std::fixed
  ...
  if (rule_manager_) {
    ...
    ss << "║   Blocked Apps:       " << std::setw(12) << rule_stats.blocked_apps
       << "                        ║\n";
    ss << "║   Blocked Domains:    " << std::setw(12)
       << rule_stats.blocked_domains << "                        ║\n";
    ss << "║   Blocked Ports:      " << std::setw(12)
...
/**
 * @brief Generates a report specifically detailing traffic classification
 * results.
 *
 * 1. What it does:
 *    Fetches classification stats from the Fast Path Manager and outputs
 * detected applications and Server Name Indications (SNIs).
 *
 * 2. Which modules depend on it:
 *    - Community 1 (DPI Engine Core): Calls this at the end of processFile()
 * alongside generateReport().
 *    - Community 3 (Frontend/Backend): Used for populating the "Detected
 * Domains/SNIs" section of the results.
 *
 * 3. What breaks if it changes:
 *    - The Node.js `packetJob.js` parser looks for the specific `[Detected
 * Domains/SNIs]` header and the `->` separator. Changing this breaks domain
 * tracking in the UI.
 *
 * 4. Known edge cases:
 *    - Can produce an empty string if the FP manager isn't initialized or no
 * flows were classified.
 *
