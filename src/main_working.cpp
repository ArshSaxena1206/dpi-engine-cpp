 ...
 * Rules are loaded typically via command-line arguments (e.g., --block-ip,
 * --block-app) and stored in memory. The `isBlocked()` method is then evaluated
 * for each packet flow to determine whether to drop or forward the packets.
 *
 ...
 * @var BlockingRules::blocked_domains List of domain substrings to match
 * against SNI.
 */
class BlockingRules {
public:
