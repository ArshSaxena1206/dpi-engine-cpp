...
void ConnectionTracker::updateConnection(Connection *conn, size_t packet_size,
                                         bool is_outbound) {
  if (!conn)
    return;
...
void ConnectionTracker::classifyConnection(Connection *conn, AppType app,
...
size_t ConnectionTracker::getActiveCount() const { return connections_.size(); }
...
void ConnectionTracker::clear() { connections_.clear(); }
...
GlobalConnectionTable::getGlobalStats() const {
  ...
  // Get top domains
  std::vector<std::pair<std::string, size_t>> domain_vec(domain_counts.begin(),
                                                         domain_counts.end());
...
std::string GlobalConnectionTable::generateReport() const {
  ...
  ss << "║ Active Connections:     " << std::setw(10)
     << stats.total_active_connections << "                          ║\n";
  ss << "║ Total Connections Seen: " << std::setw(10)
  ...
  for (const auto &pair : sorted_apps) {
    ...
    ss << "║ " << std::setw(20) << std::left << appTypeToString(pair.first)
       << std::setw(10) << std::right << pair.second << " (" << std::fixed
  ...
  if (!stats.top_domains.empty()) {
    ...
    for (const auto &pair : stats.top_domains) {
      ...
      ss << "║ " << std::setw(40) << std::left << domain << std::setw(10)
